...
model = QLearner(env, num_frames, batch_size, gamma, replay_buffer) #from dpn.py; pth file??v
...
for frame_idx in range(1, num_frames + 1): # play til 21 pts
    ...
    if len(replay_buffer) > replay_initial: # go past replay buffer
        loss = compute_td_loss(model, target_model, batch_size, gamma, replay_buffer )# target model, use by our loss? stabilizes our model
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
